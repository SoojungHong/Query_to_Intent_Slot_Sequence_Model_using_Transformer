...
#ORG - num_hiddens, num_layers, dropout, batch_size, num_steps = 32, 2, 0.0, 64, 10
num_hiddens, num_layers, dropout, batch_size, num_steps = 32, 2, 0.0, 64, 10
#num_hiddens, num_layers, dropout, batch_size, num_steps = 768, 2, 0.0, 64, 10#128, 2, 0.0, 64, 10# TEST
# ORG - lr, num_epochs, ctx = 0.005, 100, d2l.try_gpu()
lr, num_epochs, ctx = 0.005, 100, d2l.try_gpu() # TEST
ffn_num_hiddens, num_heads = 64, 4
#ffn_num_hiddens, num_heads = 128, 8    #TEST
...
def train(model, data_iter, lr, num_epochs, ctx):
...
def predict(model, src_sentence, src_vocab, tgt_vocab, num_steps, ctx):
...
"""
collected_params = model.collect_params()
for p in collected_params:
    print('debug 2 : ', p) # this is just a string
    print('debug 2-1 : ', gluon.Parameter(p).shape)
"""

train(model, train_iter, lr, num_epochs, ctx)

# save model
#model.save_parameters('saved_param.params')

# ToDo : save model and load next time without training
for sentence in ["Find a Addidas near bistro,Addias,bistro,near a bistro"]:
    print("Input Sequence : ", sentence)
    print("Output Labels : ", predict(model, sentence, src_vocab, tgt_vocab, num_steps, ctx))
